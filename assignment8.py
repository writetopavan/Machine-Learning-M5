...
#
# .. your code here ..
X=pd.read_csv('C:\\Users\\juhi\\Documents\\python\\DAT210x-master\\DAT210x-master\\Module5\\Datasets\\life_expectancy.csv',delimiter='\t')
we=X[['WhiteMale','WhiteFemale','BlackMale','BlackFemale']]
we.corr()
X.describe()
...
# WhiteFemale,BlackMale,BlackFemale
...
drawLine(model, x_train, x_train, "WhiteMale")
...
model.fit(x_train, y_train)
LinearRegression(copy_X=True, fit_intercept=True, n_jobs=1, normalize=False)
drawLine(model, x_train, x_train, "BlackFemale")
model.predict([[2014]])
#
...
plt.imshow(we.corr(), cmap=plt.cm.Blues, interpolation='nearest')
plt.colorbar()
tick_marks = [i for i in range(len(we.columns))]
plt.xticks(tick_marks, we.columns, rotation='vertical')
plt.yticks(tick_marks, we.columns)
